...
namespace VoxelImporter
{
    public abstract class VoxelBaseEditor : Editor
    {
        ...
        public static readonly GUIContent ShareSameMaterialContent = new GUIContent("Share same material", "When enabled, the same material will be shared and reduced in number.");
        #endregion
            ...
            if (baseTarget.edit_importFoldout)
            {
                ...
                if (baseTarget.advancedMode)
                {
                    ...
                    EditorGUI.indentLevel++;
                    {
                        ...
                        if (baseTarget.fileType == VoxelBase.FileType.vox)
                        {
                            ...
                            #endregion
                            #region shareSameMaterial
                            if (baseTarget.loadFromVoxelFile)
                            {
                                EditorGUI.BeginChangeCheck();
                                var shareSameMaterial = EditorGUILayout.Toggle(ShareSameMaterialContent, baseTarget.shareSameMaterial);
                                if (EditorGUI.EndChangeCheck())
                                {
                                    UndoRecordObject("Inspector");
                                    baseTarget.shareSameMaterial = shareSameMaterial;
                                    baseCore.ClearVoxelData();
                                    Refresh();
                                }
                            }
                ...
                if (baseTarget.advancedMode)
                {
                    ...
                    EditorGUI.indentLevel++;
                    {
                        ...
                        {
                            EditorGUI.BeginChangeCheck();
                            var legacySubAssetNaming = EditorGUILayout.Toggle(new GUIContent("Legacy Asset Naming", "Name the subasset a traditional name, such as 'mat0', instead of the voxel file name."), baseTarget.legacyAssetNaming);
                            if (EditorGUI.EndChangeCheck())
                            {
                                UndoRecordObject("Inspector", true);
                                baseTarget.legacyAssetNaming = legacySubAssetNaming;
                                Refresh();
                            }
                        }
                        #endregion
        ...
        protected virtual void InspectorGUI_Object_Mesh_Settings()
        {
            ...
            #endregion
            #region Generate Vertex Colors
            {
                EditorGUI.BeginChangeCheck();
                var generateVertexColors = EditorGUILayout.Toggle(new GUIContent("Generate Vertex Colors(AO)", "Generate Vertex Colors(Ambient occlusion)"), baseTarget.generateVertexColors);
                if (EditorGUI.EndChangeCheck())
                {
                    UndoRecordObject("Inspector");
                    baseTarget.generateVertexColors = generateVertexColors;
                    Refresh();
                }
            }
            if (baseTarget.generateVertexColors)
            {
                EditorGUI.indentLevel++;
                {
                    EditorGUI.BeginChangeCheck();
                    var generateVertexColorMax = EditorGUILayout.ColorField(new GUIContent("AO Color Max", "Ambient occlusion color for one face and adjacent vertices."), baseTarget.generateVertexColorMax);
                    if (EditorGUI.EndChangeCheck())
                    {
                        UndoRecordObject("Inspector");
                        baseTarget.generateVertexColorMax = generateVertexColorMax;
                        Refresh();
                    }
                }
                {
                    EditorGUI.BeginChangeCheck();
                    var generateVertexColorMin = EditorGUILayout.ColorField(new GUIContent("AO Color Min", "Ambient occlusion color for the three faces and adjacent vertices."), baseTarget.generateVertexColorMin);
                    if (EditorGUI.EndChangeCheck())
                    {
                        UndoRecordObject("Inspector");
                        baseTarget.generateVertexColorMin = generateVertexColorMin;
                        Refresh();
                    }
                }
                EditorGUI.indentLevel--;
