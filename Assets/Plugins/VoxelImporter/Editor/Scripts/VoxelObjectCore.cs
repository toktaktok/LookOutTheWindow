    ...
    public class VoxelObjectCore : VoxelBaseCore
    {
        ...
        protected override bool CreateMesh()
        {
            ...
            DisplayProgressBar("");
            {
                ...
                if (!AssetDatabase.IsMainAsset(atlasTexture))
                {
                    if (voxelBase.legacyAssetNaming)
                        atlasTexture.name = "tex";
                    else
                        atlasTexture.name = Path.GetFileNameWithoutExtension(voxelBase.voxelFilePath);
                }
                if (!AssetDatabase.Contains(atlasTexture))
                {
                    AddObjectToPrefabAsset(atlasTexture);
                }
            }
            ...
            DisplayProgressBar("");
            {
                for (int i = 0; i < materials.Count; i++)
                {
                    ...
                    if (!AssetDatabase.IsMainAsset(materials[i]))
                    {
                        if (voxelBase.legacyAssetNaming)
                            materials[i].name = string.Format("mat{0}", i);
                        else
                            materials[i].name = Path.GetFileNameWithoutExtension(voxelBase.voxelFilePath) + string.Format("_{0}", i);
                    }
                    if (!AssetDatabase.Contains(materials[i]))
                    {
                        AddObjectToPrefabAsset(materials[i]);
                    }
                }
            }
            ...
            DisplayProgressBar("");
            {
                ...
                if (!AssetDatabase.IsMainAsset(mesh))
                {
                    if (voxelBase.legacyAssetNaming)
                        ...
                        mesh.name = Path.GetFileNameWithoutExtension(voxelBase.voxelFilePath);
                }
                if (!AssetDatabase.Contains(mesh))
                {
                    AddObjectToPrefabAsset(mesh);
                }
            }
