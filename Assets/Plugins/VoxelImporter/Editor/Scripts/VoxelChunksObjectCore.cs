                ...
                }
                {
                    ...
                    for (int i = 0; i < chunkVoxels.Length; i++)
                    {
                        ...
                        if (voxelBase.legacyAssetNaming)
                            data.name = string.Format("Chunk({0})", voxelData.chunkDataList[i].name);
                        else
                            data.name = voxelData.chunkDataList[i].name;
            ...
            else
            {
                ...
                }
                {
                    ...
                    chunkVoxels.AllAction((x, y, z, list) =>
                    {
                        ...
                        if (voxelBase.legacyAssetNaming)
                            data.name = string.Format("Chunk({0}, {1}, {2})", x, y, z);
                        else
                            data.name = string.Format("{0}, {1}, {2}", x, y, z);
                        chunkDataList.Add(data);
                    });
        ...
        protected override bool CreateMesh()
        {
            ...
            else if (voxelObject.materialMode == VoxelChunksObject.MaterialMode.Individual)
            {
                ...
                for (int c = 0; c < chunkDataList.Count; c++)
                {
                    ...
                    if (!AssetDatabase.IsMainAsset(chunkDataList[c].chunkObject.atlasTexture))
                    {
                        if (voxelBase.legacyAssetNaming)
                            chunkDataList[c].chunkObject.atlasTexture.name = string.Format("{0}_tex", chunkDataList[c].chunkObject.name);
                        else
                            chunkDataList[c].chunkObject.atlasTexture.name = Path.GetFileNameWithoutExtension(voxelBase.voxelFilePath) + string.Format("_{0}", chunkDataList[c].chunkObject.name);
                    }
                    if (!AssetDatabase.Contains(chunkDataList[c].chunkObject.atlasTexture))
                    {
                        AddObjectToPrefabAsset(chunkDataList[c].chunkObject.atlasTexture);
                    }
                }
                #endregion
            }
                ...
                else if (voxelObject.materialMode == VoxelChunksObject.MaterialMode.Individual)
                {
                    ...
                    for (int c = 0; c < chunkDataList.Count; c++)
                    {
                        ...
                        for (int i = 0; i < chunkDataList[c].chunkObject.materials.Count; i++)
                        {
                            ...
                            if (!AssetDatabase.IsMainAsset(chunkDataList[c].chunkObject.materials[i]))
                            {
                                if (voxelBase.legacyAssetNaming)
                                    chunkDataList[c].chunkObject.materials[i].name = string.Format("{0}_mat{1}", chunkDataList[c].chunkObject.name, i);
                                else
                                    chunkDataList[c].chunkObject.materials[i].name = Path.GetFileNameWithoutExtension(voxelBase.voxelFilePath) + string.Format("_{0}_{1}", chunkDataList[c].chunkObject.name, i);
                            }
                            if (!AssetDatabase.Contains(chunkDataList[c].chunkObject.materials[i]))
                            {
                                AddObjectToPrefabAsset(chunkDataList[c].chunkObject.materials[i]);
                            }
                        }
                    }
                    #endregion
            ...
            DisplayProgressBar("");
            {
                ...
                for (int i = 0; i < chunkDataList.Count; i++)
                {
                    if (!AssetDatabase.IsMainAsset(chunkDataList[i].chunkObject.mesh))
                    {
                        if (voxelBase.legacyAssetNaming)
                            chunkDataList[i].chunkObject.mesh.name = string.Format("{0}_mesh", chunkDataList[i].chunkObject.name);
                        else
                            chunkDataList[i].chunkObject.mesh.name = Path.GetFileNameWithoutExtension(voxelBase.voxelFilePath) + string.Format("_{0}", chunkDataList[i].chunkObject.name);
                    }
                    if (!AssetDatabase.Contains(chunkDataList[i].chunkObject.mesh))
                    {
                        AddObjectToPrefabAsset(chunkDataList[i].chunkObject.mesh);
                    }
                }
            }
