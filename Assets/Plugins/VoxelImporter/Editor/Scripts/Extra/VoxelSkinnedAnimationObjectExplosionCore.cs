    ...
    public class VoxelSkinnedAnimationObjectExplosionCore : VoxelBaseExplosionCore
    {
...

            {
                    ...
                    if (vertices.Count + cubeVertices.Count >= 65000)
                    {
                        ...
                        explosionObject.meshes[meshIndex] = CreateMesh(explosionObject.meshes[meshIndex]);
                        {
                            if (voxelBase.legacySubAssetNaming)
                                explosionObject.meshes[meshIndex].mesh.name = string.Format("explosion_mesh{0}", meshIndex);
                            else
                                explosionObject.meshes[meshIndex].mesh.name = Path.GetFileNameWithoutExtension(voxelBase.voxelFilePath) + string.Format("_explosion_{0}", meshIndex);
                        }
                        if (!AssetDatabase.Contains(explosionObject.meshes[meshIndex].mesh))
                        {
                            voxelBaseCore.AddObjectToPrefabAsset(explosionObject.meshes[meshIndex].mesh);
                        }
                        meshIndex++;
                    }
#endif
                ...
                if (vertices.Count > 0)
                {
                    for (int i = explosionObject.meshes.Count; i <= meshIndex; i++)
                        explosionObject.meshes.Add(null);
                    explosionObject.meshes[meshIndex] = CreateMesh(explosionObject.meshes[meshIndex]);
                    if (!AssetDatabase.IsMainAsset(explosionObject.meshes[meshIndex].mesh))
                    {
        ...
        public void Bake()
        {
            ...
            for (int i = 0; i < explosionObject.meshes.Count; i++)
            {
                ...
                {
                    if (!AssetDatabase.IsMainAsset(explosionObject.meshes[i].bakeMesh))
                    {
                        if (voxelBase.legacyAssetNaming)
                            explosionObject.meshes[i].bakeMesh.name = string.Format("explosion_bake_mesh{0}", i);
                        else
                            explosionObject.meshes[i].bakeMesh.name = Path.GetFileNameWithoutExtension(voxelBase.voxelFilePath) + string.Format("_explosion_bake_{0}", i);
                    }
                    if (!AssetDatabase.Contains(explosionObject.meshes[i].bakeMesh))
                    {
                        voxelBaseCore.AddObjectToPrefabAsset(explosionObject.meshes[i].bakeMesh);
                    }
                }
            }
        }
